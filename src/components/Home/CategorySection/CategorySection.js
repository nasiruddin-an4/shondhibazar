          ...
          {categories.map((category, index) => (
            ...
            >
              <Link href={category.href} className="block group">
                <motion.div
                  className="flex flex-col items-center p-4 bg-white rounded-lg transition-all duration-300 hover:shadow-lg"
                  whileHover={{ y: -5 }}
                >
                  {/* Icon Container */}
                  <div className="w-12 h-12 md:w-16 md:h-16 mb-3 text-gray-600 transition-all duration-300 group-hover:text-green-600 group-hover:scale-110">
                    {category.icon}
                  </div>
                  {/* Category Name */}
