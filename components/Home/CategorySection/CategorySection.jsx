...
// Hand-drawn icons for the categories we know about today. Any category not listed here
// (or added later in the admin) falls back to DEFAULT_ICON rather than breaking the section.
const ICONS_BY_NAME = {
  "Rice & Grains": (
    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" className="w-full h-full">
      <path d="M6.5 8.5c0-3.5 2.5-6.5 5.5-6.5s5.5 3 5.5 6.5-2.5 4.5-5.5 4.5-5.5-1-5.5-4.5z" strokeWidth="1.5" />
  ...
  "Spices & Hearbs": (
    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" className="w-full h-full">
      <path d="M12 8c-2.2 0-4 1.8-4 4s1.8 4 4 4 4-1.8 4-4-1.8-4-4-4z" strokeWidth="1.5" />
      <path d="M12 4C7.6 4 4 7.6 4 12s3.6 8 8 8 8-3.6 8-8-3.6-8-8-8z" strokeWidth="1.5" />
      <path d="M12 15c-1.7 0-3-1.3-3-3s1.3-3 3-3 3 1.3 3 3-1.3 3-3 3z" strokeWidth="1.5" />
    </svg>
  ),
  "Sugar & Honey": (
    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" className="w-full h-full">
      ...
      <div className="container mx-auto px-4">
        <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-8 gap-4 md:gap-6">
          {categories.map((category, index) => (
            ...
            >
              <Link href={`/product-category?category=${encodeURIComponent(category.name)}`} className="block group h-full">
                <motion.div
                ...
                >
                  <div className="w-10 h-10 md:w-12 md:h-12 mb-3 text-gray-600 transition-all duration-300 group-hover:text-green-600 group-hover:scale-110">
                    {ICONS_BY_NAME[category.name] || DEFAULT_ICON}
                  </div>
